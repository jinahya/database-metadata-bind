...
cat <<EOF
ExternalIT command:
cd '${PROJECT_DIR}' && ${MVN} -Pfailsafe,external-oracle -Dit.test=ExternalIT -Durl='${URL}' -Duser='${ORACLE_USER}' -Dpassword='${PASSWORD}' test-compile failsafe:integration-test
...
(cd "${PROJECT_DIR}" && "${MVN}" -Pfailsafe,external-oracle \
  -Dit.test=ExternalIT \
  -Durl="${URL}" \
  -Duser="${ORACLE_USER}" \
  -Dpassword="${PASSWORD}" \
