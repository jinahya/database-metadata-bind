...
cat <<EOF
ExternalIT command:
cd '${PROJECT_DIR}' && ${MVN} -Pfailsafe,external-mysql -Dit.test=ExternalIT -Durl='${URL}' -Duser='${MYSQL_USER}' -Dpassword='${PASSWORD}' test-compile failsafe:integration-test
...
(cd "${PROJECT_DIR}" && "${MVN}" -Pfailsafe,external-mysql \
  -Dit.test=ExternalIT \
  -Durl="${URL}" \
  -Duser="${MYSQL_USER}" \
  -Dpassword="${PASSWORD}" \
